...
public abstract class TransactionType {
    ...
    final boolean applyUnconfirmed(Transaction transaction, Account senderAccount) {
        ...
        /*如果发送者账号中未证实的BalanceNQT<totalAmountNQT *交易的时间戳等于0且交易的创建者等于Genesis.CREATOR_PUBLIC_KEY 则直接验证通过*/
        if (senderAccount.getUnconfirmedBalanceNQT() < totalAmountNQT
                && ! (transaction.getTimestamp() == 0 && Arrays.equals(senderAccount.getPublicKey(), Genesis.CREATOR_PUBLIC_KEY))) {
            return false;
        }
        /*从发送者账号中减去发送的总的totalAmountNQT*/
        senderAccount.addToUnconfirmedBalanceNQT(- totalAmountNQT);
        /*验证未确定的附件，如果没有通过，则往发送者账号中增加从UnconfirmedBalanceNQT减去的totalAmountNQT*/
     ...
     *
     * @param transaction
     * @param accumulatedAmounts
     * @param accumulatedAssetQuantities
