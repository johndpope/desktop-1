...
public final class GetAccount extends APIServlet.APIRequestHandler {
    ...
    JSONStreamAware processRequest(HttpServletRequest req) throws NxtException {
        ...
        synchronized (account) {
            ...
            Logger.logMessage("获取账户相关：account="+ Convert.toUnsignedLong(account.getId()));
            Logger.logMessage("获取账户相关：accountRS="+ Convert.toUnsignedLong(account.getId()));
            response.put("account", Convert.toUnsignedLong(account.getId()));
            response.put("accountRS", Convert.rsAccount(account.getId()));
            Logger.logMessage("Account 的publickey="+account.getPublicKey());
            if (account.getPublicKey() != null) {
                Logger.logMessage("获取账号相关：publicKey= Convert.toHexString(account.getPublicKey())");
